...
/**
 * @brief 文字列Tから構築されたDynamicStringについて、ランダムなiに対して
 *        dynRLSLP::DynamicRLSLPString::access(i) == T[i]が成り立つことを検証するテスト関数
...
bool access_test(bool use_restricted_block_compression, int64_t seed, uint64_t alphabet_type, dynRLSLP::DictionaryMode mode, uint64_t num_trials = 1000, uint64_t max_length = 10000)
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "DynamicString access テスト実行中" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "  3. ランダムな位置iを選択 (0 <= i < T.length())" << std::endl;
    std::cout << "  4. dynRLSLP::DynamicRLSLPString::access(i) == T[i] を検証" << std::endl;
    std::cout << std::endl;
    ...
    for (uint64_t trial = 0; trial < num_trials; trial++)
    {
        ...
        try
        {
            ...
            for (uint64_t i = 0; i < length; i++)
            {
                ...
                {
                    failed_tests++;
                    std::cout << "✗ テスト " << (trial + 1) << " 失敗 (長さ: " << length << ", 位置: " << i << ")" << std::endl;
                    std::cout << "  期待値: '" << (char)expected_char << "' (" << (int)expected_char << ")" << std::endl;
                    std::cout << "  実際の値: '" << (char)accessed_char << "' (" << (int)accessed_char << ")" << std::endl;
        ...
        if ((trial + 1) % 100 == 0)
        {
            std::cout << "進捗: " << (trial + 1) << "/" << num_trials << " (成功: " << passed_tests << ", 失敗: " << failed_tests << ")" << std::endl;
        }
    }
...
int main(int argc, char *argv[])
{
    // テストの説明を表示
    std::cout << "========================================" << std::endl;
    std::cout << "dynRLSLP::DynamicRLSLPString::access() メソッドのテスト" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    std::cout << "【テストの目的】" << std::endl;
    std::cout << "文字列Tから構築されたDynamicStringについて、" << std::endl;
    std::cout << "ランダムな位置iに対して dynRLSLP::DynamicRLSLPString::access(i) == T[i] が" << std::endl;
    ...
    std::cout << "3. 各位置iを選択します（0 <= i < T.length()）" << std::endl;
    std::cout << "4. dynRLSLP::DynamicRLSLPString::access(i) と T[i] を比較します" << std::endl;
