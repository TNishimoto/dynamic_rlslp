...
bool delete_test_int64(bool use_restricted_block_compression, int64_t seed, uint64_t alphabet_type, dynRLSLP::DictionaryMode mode, uint64_t num_trials = 1000, uint64_t num_deletes_per_string = 10, uint64_t max_length = 10000)
{
    ...
    for (uint64_t trial = 0; trial < num_trials; trial++)
    {
        ...
        for (uint64_t delete_op = 0; delete_op < num_deletes_per_string; delete_op++)
        {
            ...
            uint64_t min_len = std::min<uint64_t>(100, text.size());
            std::uniform_int_distribution<int64_t> len_dist(0, static_cast<int64_t>(min_len));
            int64_t len2 = len_dist(mt);
            std::uniform_int_distribution<int64_t> pos_dist(0, static_cast<int64_t>(text.size() - len2));
            int64_t pos = pos_dist(mt);

            ds.delete_substring(pos, len2);
            text.erase(text.begin() + pos, text.begin() + pos + len2);
            ...
            {
                failed_tests++;
                std::cout << "Delete mismatch at trial " << (trial + 1) << ", op " << (delete_op + 1) << ", pos " << pos << ", len " << len2 << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if ((arg == "--deletes-per-string" || arg == "-d") && i + 1 < argc)
        {
    ...
    if (alphabet_type == 0)
    {
        ...
        {
            all_success = delete_test_int64(use_restricted_block_compression, seed, type, mode, num_trials, num_deletes_per_string, max_length) && all_success;
    ...
    {
        all_success = delete_test_int64(use_restricted_block_compression, seed, alphabet_type, mode, num_trials, num_deletes_per_string, max_length);
    }
