...
/**
 * @brief 2つの明示的非終端が導出する文字列の最長共通接頭辞の長さ（ナイーブ実装）
 */
uint64_t naive_lcp(
...
bool lcp_test(bool use_restricted_block_compression, int64_t seed, uint64_t alphabet_type, dynRLSLP::DictionaryMode mode, uint64_t num_trials = 1000, uint64_t max_length = 10000)
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "DynamicString lcp テスト実行中" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "  3. 各非Null明示的非終端の組 (X1, X2) について lcp(X1, X2) を検証" << std::endl;
    std::cout << "  4. dynRLSLP::DynamicRLSLPString::lcp(X1, X2) == naive_lcp(X1, X2) を確認" << std::endl;
    std::cout << std::endl;
...
{
    std::cout << "========================================" << std::endl;
    std::cout << "dynRLSLP::DynamicRLSLPString::lcp() のテスト" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    std::cout << "【テストの目的】" << std::endl;
    std::cout << "明示的非終端 X1, X2 が導出する2つの文字列の最長共通接頭辞の長さについて、" << std::endl;
    std::cout << "DynamicRLSLPString::lcp(X1, X2) がナイーブ実装と一致することを検証します。" << std::endl;
    std::cout << std::endl;
