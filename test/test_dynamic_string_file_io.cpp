...
/**
 * @brief Test file save/load for randomly generated std::vector<uint8_t> T
 ...
 */
bool file_io_test(bool use_restricted_block_compression, int64_t seed, uint64_t alphabet_type, dynRLSLP::DictionaryMode mode, uint64_t num_trials = 1000, uint64_t max_length = 10000)
{
    // Get alphabet
    std::vector<uint8_t> alphabet = get_alphabet_by_type(alphabet_type);

    std::cout << "========================================" << std::endl;
    std::cout << "Running DynamicString file save/load test" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "  Step 1: Generate random string T (length uniformly random from 1 to max_length)" << std::endl;
    std::cout << "  Step 2: Build DynamicString ds from T" << std::endl;
    std::cout << "  Step 3: Save ds to file with dynRLSLP::DynamicRLSLPString::store_to_file(ds, x)" << std::endl;
    std::cout << "  Step 4: Restore ds_prime with dynRLSLP::DynamicRLSLPString::load_from_file(y)" << std::endl;
    std::cout << "  Step 5: Verify ds and ds_prime are nearly equal with dynRLSLP::DynamicRLSLPString::verify_nearly_equal" << std::endl;
    std::cout << std::endl;
    ...
    {
        // Step 1: Generate random string T
        ...
        std::string temp_file = temp_dir + "/dynamic_string_test_" + std::to_string(trial) + "_" + std::to_string(seed) + ".tmp";

        try
        {
            // Step 2: Build DynamicString ds from T
            ...
            // Step 3: Save ds to file with dynRLSLP::DynamicRLSLPString::store_to_file(ds, x)
            std::ofstream x(temp_file, std::ios::binary);
            if (!x.is_open())
            {
                failed_tests++;
                std::cout << "✗ Test " << (trial + 1) << " file open failed (file: " << temp_file << ")" << std::endl;
                continue;
            }
            dynRLSLP::DynamicRLSLPString::store_to_file(ds, x);
            x.close();

            // Step 4: Restore ds_prime with dynRLSLP::DynamicRLSLPString::load_from_file(y)
            std::ifstream y(temp_file, std::ios::binary);
            if (!y.is_open())
            {
                failed_tests++;
                std::cout << "✗ Test " << (trial + 1) << " file read open failed (file: " << temp_file << ")" << std::endl;
                // Delete file
                std::remove(temp_file.c_str());
                continue;
            }
            dynRLSLP::DynamicRLSLPString ds_prime = dynRLSLP::DynamicRLSLPString::load_from_file(y);
            y.close();

            // Delete temporary file
            std::remove(temp_file.c_str());

            // Step 5: Verify ds and ds_prime are nearly equal
            bool is_equal = ds.verify_nearly_equal(ds_prime);
            ...
            {
                failed_tests++;
                std::cout << "✗ Test " << (trial + 1) << " verification failed (length: " << length << ")" << std::endl;
                ...
                if (failed_tests <= 10)
                {
                    ...
                    std::cout << "  ds.get_text_str(): \"" << ds.get_text_str() << "\"" << std::endl;
                    std::cout << "  ds_prime.get_text_str(): \"" << ds_prime.get_text_str() << "\"" << std::endl;
                }
            }
        ...
        {
            failed_tests++;
            std::cout << "✗ Test " << (trial + 1) << " logic error exception (length: " << length << "): " << e.what() << std::endl;
            ...
            failed_tests++;
            std::cout << "✗ Test " << (trial + 1) << " runtime error exception (length: " << length << "): " << e.what() << std::endl;
            // Delete file if it still exists
            if (std::filesystem::exists(temp_file))
            {
                std::remove(temp_file.c_str());
        ...
        if ((trial + 1) % 100 == 0)
        {
            std::cout << "Progress: " << (trial + 1) << "/" << num_trials << " (passed: " << passed_tests << ", failed: " << failed_tests << ", |T|: " << last_length << ")" << std::endl;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "Test for dynRLSLP::DynamicRLSLPString::store_to_file() / load_from_file()" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    std::cout << "[Test purpose]" << std::endl;
    std::cout << "For a randomly generated std::vector<uint8_t> T," << std::endl;
    std::cout << "save the DynamicString to a file, load it back," << std::endl;
    std::cout << "and verify it is nearly equal to the original DynamicString." << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "3. Save to file with dynRLSLP::DynamicRLSLPString::store_to_file()" << std::endl;
    std::cout << "4. Restore from file with dynRLSLP::DynamicRLSLPString::load_from_file()" << std::endl;
    std::cout << "5. Compare with the original instance using dynRLSLP::DynamicRLSLPString::verify_nearly_equal()" << std::endl;
    std::cout << "6. Repeat the above for num_trials trials" << std::endl;
    std::cout << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    if (alphabet_type == 0)
    {
        ...
        for (uint64_t type = 1; type <= 4; type++)
        {
            ...
            bool success = file_io_test(use_restricted_block_compression, seed, type, mode, num_trials, max_length);
    ...
    else
    {
        ...
        all_success = file_io_test(use_restricted_block_compression, seed, alphabet_type, mode, num_trials, max_length);
