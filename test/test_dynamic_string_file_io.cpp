...
/**
 * @brief ランダムに生成したstd::vector<uint8_t> Tに対してファイル保存・読み込みのテストを行う関数
 * @param use_restricted_block_compression 制限付きブロック圧縮を使用するかどうか
 * @param seed 乱数のシード値
 * @param alphabet_type アルファベットタイプ (1-4)
 * @param mode DictionaryMode
 * @param num_trials テストの試行回数（デフォルト: 1000）
...
bool file_io_test(bool use_restricted_block_compression, int64_t seed, uint64_t alphabet_type, dynRLSLP::DictionaryMode mode, uint64_t num_trials = 1000, uint64_t max_length = 10000)
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "DynamicString ファイル保存・読み込み テスト実行中" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "  Step 1: ランダムな文字列Tを生成（長さは1からmax_lengthまでランダム）" << std::endl;
    std::cout << "  Step 2: TからDynamicString dsを構築" << std::endl;
    std::cout << "  Step 3: dynRLSLP::DynamicRLSLPString::store_to_file(ds, x)でdsをファイルに保存" << std::endl;
    std::cout << "  Step 4: dynRLSLP::DynamicRLSLPString::load_from_file(y)でds_primeを復元" << std::endl;
    std::cout << "  Step 5: dynRLSLP::DynamicRLSLPString::verify_nearly_equalでdsとds_primeがほぼ同じ内容か検証" << std::endl;
    std::cout << std::endl;
    ...
    for (uint64_t trial = 0; trial < num_trials; trial++)
    {
        ...
        try
        {
            ...
            // Step 3: dynRLSLP::DynamicRLSLPString::store_to_file(ds, x)でdsをファイルに保存
            std::ofstream x(temp_file, std::ios::binary);
            if (!x.is_open())
            {
                failed_tests++;
                std::cout << "✗ テスト " << (trial + 1) << " ファイルオープン失敗 (ファイル: " << temp_file << ")" << std::endl;
                continue;
            }
            dynRLSLP::DynamicRLSLPString::store_to_file(ds, x);
            x.close();

            // Step 4: dynRLSLP::DynamicRLSLPString::load_from_file(y)でds_primeを復元
            std::ifstream y(temp_file, std::ios::binary);
            if (!y.is_open())
            {
                failed_tests++;
                std::cout << "✗ テスト " << (trial + 1) << " ファイル読み込みオープン失敗 (ファイル: " << temp_file << ")" << std::endl;
                // ファイルを削除
            ...
            // Step 5: dynRLSLP::DynamicRLSLPString::verify_nearly_equalでdsとds_primeがほぼ同じ内容か検証
            ...
            {
                failed_tests++;
                std::cout << "✗ テスト " << (trial + 1) << " 検証失敗 (長さ: " << length << ")" << std::endl;
        ...
        {
            failed_tests++;
            std::cout << "✗ テスト " << (trial + 1) << " ロジックエラー例外発生 (長さ: " << length << "): " << e.what() << std::endl;
            // ファイルが残っている場合は削除
            ...
            failed_tests++;
            std::cout << "✗ テスト " << (trial + 1) << " ランタイムエラー例外発生 (長さ: " << length << "): " << e.what() << std::endl;
            // ファイルが残っている場合は削除
            if (std::filesystem::exists(temp_file))
            {
                std::remove(temp_file.c_str());
        ...
        if ((trial + 1) % 100 == 0)
        {
            std::cout << "進捗: " << (trial + 1) << "/" << num_trials << " (成功: " << passed_tests << ", 失敗: " << failed_tests << ", Tの長さ: " << last_length << ")" << std::endl;
        }
    }
...
int main(int argc, char *argv[])
{
    // テストの説明を表示
    std::cout << "========================================" << std::endl;
    std::cout << "dynRLSLP::DynamicRLSLPString::store_to_file() / load_from_file() メソッドのテスト" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    std::cout << "【テストの目的】" << std::endl;
    std::cout << "ランダムに生成したstd::vector<uint8_t> Tに対して、" << std::endl;
    std::cout << "DynamicStringをファイルに保存し、読み込んだ後、" << std::endl;
    std::cout << "元のDynamicStringとほぼ同じ内容であることを検証します。" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "3. dynRLSLP::DynamicRLSLPString::store_to_file()でファイルに保存します" << std::endl;
    std::cout << "4. dynRLSLP::DynamicRLSLPString::load_from_file()でファイルから復元します" << std::endl;
    std::cout << "5. dynRLSLP::DynamicRLSLPString::verify_nearly_equal()で元のインスタンスと比較します" << std::endl;
    std::cout << "6. 上記の手順をnum_trials回繰り返します" << std::endl;
    std::cout << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (arg == "--help" || arg == "-h")
        {
            ...
            std::cout << "  --trials, -t <値>             テストの試行回数を指定 (デフォルト: 1000)" << std::endl;
