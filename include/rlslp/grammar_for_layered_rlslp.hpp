...
namespace dynRLSLP
{
	...
	public:
		...
		StaticRLSLP convert_to_canonical_rlslp() const
		{
			...
			std::sort(id_list.begin(), id_list.end(), [&](uint64_t a, uint64_t b)
					  { return explicit_nonterminal_length_list[a] < explicit_nonterminal_length_list[b]; });
			...
			r.length_list.resize(r.nonterminal_list.size(), UINT64_MAX);
			r.height_list.resize(r.nonterminal_list.size(), UINT16_MAX);

			for (uint64_t i = 0; i < r.nonterminal_list.size(); i++)
			{
				if (r.rule_list[i].get_type() == RLSLPRuleType::Character)
				{
					r.length_list[i] = 1;
					r.height_list[i] = 0;
				}
				else if (r.rule_list[i].get_type() == RLSLPRuleType::Pair)
				{
					r.length_list[i] = r.length_list[r.rule_list[i].A] + r.length_list[r.rule_list[i].B];
					r.height_list[i] = std::max(r.height_list[r.rule_list[i].A], r.height_list[r.rule_list[i].B]) + 1;
				}
				else if (r.rule_list[i].get_type() == RLSLPRuleType::Power)
				{
					r.length_list[i] = r.length_list[r.rule_list[i].A] * r.rule_list[i].B;
					r.height_list[i] = r.height_list[r.rule_list[i].A] + 1;
				}
				else
				{
					throw std::runtime_error("The rule type is not supported.");
		...
		void write_content_as_json_format(std::ofstream &ofs, std::string name, int64_t message_paragraph = stool::Message::SHOW_MESSAGE) const
		{
			...
			JsonHelper::write_content_as_json_format<NonterminalWithRelativeLevel, uint64_t>(
				...
				[](const NonterminalWithRelativeLevel &key)
				{ return NonterminalFunctions::to_string(key); },
